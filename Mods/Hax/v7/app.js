if ('undefined' != typeof self.hax) throw 'abort: Patch already installed';
self.hax_installed = false, self.hax = {
  ...
  'sendListeners': new Set(['debug']),
  'recvListeners': new Set(['debug']),
  'chatListeners': new Set(['debug']),
  'commands': new Set(['/help', '/status', '/debug', '/a']),
  'plugins': {
    'register': function (l, r) {
      Object.assign(this[l] = {}, {
        'enabled': false
      }, r), r.chatListener && hax.chatListeners.add(l), r.recvListener && hax.recvListeners.add(l), r.sendListener && hax.sendListeners.add(l), r.command && hax.commands.add('/' + l), r.init && this[l].init();
    },
    'toggle': function (l) {
    ...
    'help': {
      'command': function (l) {
		append('');
		append('ML Patch by Anonym');
		append('Version: '+ hax.version);
		append('Commands:');
		let j = hax.plugins.secret.allowed();
		Object.values(hax.plugins).forEach(Y => j && Y.sdoc ? append(Y.sdoc) : Y.doc ? append(Y.doc) : null);
    ...
    'debug': {
      ...
      'command': function (l) {
        if (l[1]) {
			let param = l[1].toLowerCase();
			if (param.match(/(c|m|s)/)) {
				this[l[1]] = !this[l[1]];
				append('Debug [' + param + ']: ' + (this[l[1]] ? 'ON' : 'OFF'));
			}
        }
      },
      'sendListener': function (l) {
        this.s && console.log('Send:', l);
      },
      'recvListener': function (l) {
        this.r && console.log('Recv:', l);
      },
      'chatListener': function (l) {
        this.c && console.log('Chat:', l);
      },
    },
    'a': {
      'command': () => hax.go()
  ...
  'init': function () {
    hax_installed || (self.old_append = self.append, self.append = function (l) {
      ...
      try {
        hax.recvListeners.forEach(r => hax.plugins[r].recvListener(l));
      } catch (r) {
        if ('spam' == r) return;
        throw r;
      }
      ...
      try {
        hax.sendListeners.forEach(r => hax.plugins[r].sendListener(l));
      } catch (r) {
        if ('spam' == r) return;
